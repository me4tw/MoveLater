/**@file mvafter.c
@brief Schedule file move after a system reboot.

This simple console mode application will set up files or
directories to be renamed, deleted, or replaced at bootime.
This is particularly useful when replacing files that are
persistently in use when the operating system is booted.
MvAfter also displays and clears pending move operations.

#USAGE

###MVAFTER
Enumerates pending move operations.
###MVAFTER source destination
...
Moving of a directory to another non-existant one on the
same partition is supported (this may be considered a
rename).

Moving of files from one volume to another is not supported
under NT/2k. You must first copy the file to the volume as
the destination, and then use MvAfter.

Deletion of directories will not occur unless the
